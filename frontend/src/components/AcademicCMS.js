...
const AcademicCMS = () => {
  ...
  // Navigation states for hierarchical flow (Updated)
  ...
  const handleChapterChange = (
    ...
    setForm((prev) => {
      ...
      newChapters[index] = {
        ...
        // Clear file name if URL is cleared
  ...
  const handleAddBook = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...bookForm,
        // Filter out empty/unfiled chapters
        chapters: bookForm.chapters.filter(
          (c) => c.title.trim() && c.file_url.trim(),
        ),
        // Map old fields to new
        pdf_url: bookForm.prelims_file_url,
        cover_image_url: bookForm.prelims_file_url, // Use prelims URL for cover image fallback
      };
  ...
  const handleAddReferenceBook = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        pdf_url: referenceBookForm.prelims_file_url, // Map to old pdf_url/file_url fields if needed
      };
    ...
    try {
      ...
      const qaData = {
        ...
        keywords: qaForm.keywords
          ...
          .map((k) => k.trim())
          .filter((k) => k),
        // Map chapter to chapter_topic
        chapter_topic: qaForm.chapter || "",
      };

      const endpoint = `${API_BASE_URL}/cms/qa-knowledge-base`; // Renamed endpoint
  ...
  const handleEditQA = (qa) => {
    ...
    setQaForm({
      ...
      keywords: Array.isArray(qa.keywords)
        ? qa.keywords.join(", ")
        : qa.keywords || "",
      chapter: qa.chapter_topic || "", // Map chapter_topic back to chapter for form
    });
    setShowAddQA(true);
  };
  ...
  const handleAddPaper = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const endpoint = `${API_BASE_URL}/cms/previous-year-papers`; // Renamed endpoint
  ...
  // --- Common Book/Reference Book Modal Renderer (Updated) ---
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div className="bg-white rounded-lg p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleAddFunction} className="space-y-4">
            ...
            {/* Dynamic Chapters Upload (Max 20) */}
            <div className="space-y-3 p-4 border rounded-lg bg-white shadow-inner">
              ...
              {formState.chapters.map((chapter, index) => (
                ...
                >
                  ...
                  {/* File Status / Clear / Remove Button Group */}
            ...
            {/* BULK UPLOAD for Chapters (Max 100MB) */}
            <div className="border p-4 rounded-lg bg-gray-50">
    ...
    <div className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {books.map((book) => (
              ...
              >
                ...
                {book.prelims_file_name && (
                  <a
                    href={book.prelims_file_url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-1 bg-yellow-100 text-yellow-800 text-xs px-2 py-1 rounded hover:bg-yellow-200 mt-2"
                  >
      ...
      {activeTab === "reference" && (
        <div>
          ...
          {/* Step 2: Select Subject / View Books */}
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    ...
                    <ul className="text-sm text-blue-600 list-disc list-inside space-y-1">
                      ...
                      <li>
                        <strong>chapter_topic</strong> (optional) - e.g., Laws
                        of Motion
                      </li>
                      <li>

...
const ClassManagement = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const teachers = staffRes.data.filter(
        (s) =>
          s.designation &&
  ...
  const handleClassSubmit = async (e) => {
    ...
    try {
      ...
      if (editingClass) {
        ...
        await axios.post(`${API}/classes`, submitData, { headers });
  ...
  const handleSectionSubmit = async (e) => {
    ...
    try {
      ...
      if (editingSection) {
        ...
        await axios.post(`${API}/sections`, submitData, { headers });
  ...
  const handleEditSection = (section) => {
    setSectionFormData({
      class_id: section.class_id || "select_class",
      name: section.name,
  ...
  const handleDeleteClass = async (cls) => {
    ...
    try {
      const headers = getAuthHeaders();
      await axios.delete(`${API}/classes/${cls.id}`, { headers });
  ...
  const handleDeleteSection = async (section) => {
    ...
    try {
      const headers = getAuthHeaders();
      await axios.delete(`${API}/sections/${section.id}`, { headers });
  ...
  const createSampleTeachers = async () => {
    try {
      ...
      for (const teacher of sampleTeachers) {
        try {
          await axios.post(`${API}/staff`, teacher, { headers });
                ...
                <form onSubmit={handleClassSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setClassFormData({ ...classFormData, standard: value })
                      }
                    >
                      <SelectTrigger>

...
// Used only if class API fails, so UI still works
const FALLBACK_CLASSES = ["5", "6", "7", "8", "9", "10", "11", "12"];
...
const QuizTool = () => {
  ...
  // ==============================
  // Fetch classes from Class API
  // ==============================
  const fetchClasses = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get(`${API_BASE_URL}/classes/list`, {
      ...
      const mapped = (raw || []).map((c) => ({
        ...
        label:
          ...
          c.class_name ??
          c.title ??
          `Class ${c.class_standard || c.standard || c.name || c.id}`,
      }));

      if (!mapped.length) {
        throw new Error("Empty class list");
      }
  ...
  // ==============================
  // Generate quiz (chapter compulsory)
  // ==============================

  const handleGenerateQuiz = async () => {
    if (!filters.class_standard || !filters.subject || !filters.chapter) {
            ...
            <div>
              ...
              >
                ...
                {classOptions.map((cls) => (
                  <option key={cls.id || cls.value} value={cls.value}>
                    {cls.label}
                  </option>
                ))}
              </select>

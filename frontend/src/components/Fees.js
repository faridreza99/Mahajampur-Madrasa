...
const Fees = () => {
  ...
  const currentTab =
    tabPath === "student-specific"
      ? "student-specific"
      : tabPath === "due"
        ? "due"
        : tabPath === "select-student"
          ? "select-student"
          : tabPath === "collection"
            ? "collection"
            : "manage";
  ...
  const [showGenerateReportsModal, setShowGenerateReportsModal] =
    useState(false);
    ...
    try {
      ...
      if (response.data) {
        setSchoolBranding({
          ...
          school_name_bn:
            response.data.school_name_bn || response.data.school_name || "",
  ...
  const loadFeeDataFromBackend = async () => {
    try {
      ...
      const [configsRes, dashboardRes, paymentsRes, dueFeesRes] =
        await Promise.all([
      ...
      const transformedConfigs = {
        "Tuition Fees": backendConfigs.filter(
          (c) => c.fee_type === "Tuition Fees",
        ),
        "Transport Fees": backendConfigs.filter(
          (c) => c.fee_type === "Transport Fees",
        ),
        "Admission Fees": backendConfigs.filter(
          (c) => c.fee_type === "Admission Fees",
        ),
      };
      ...
      const transformedPayments = paymentsData.map((payment) => ({
        ...
        student_name:
          ...
          payment.student_name.trim() !== "" &&
          payment.student_name !== "Unknown Student"
            ? payment.student_name
            : payment.student_id
              ? `ছাত্র #${String(payment.student_id).slice(-6)}`
              : "অজ্ঞাত ছাত্র",
  ...
  const calculateTotalsFromConfigurations = (configs) => {
        ...
        const frequencyMultiplier =
          config.frequency === "monthly"
            ...
            : config.frequency === "quarterly"
              ? 4
              : config.frequency === "half-yearly"
                ? 2
                : 1;

        totalCalculated +=
          config.amount * classMultiplier * frequencyMultiplier;
      });
    });
    ...
    return {
      ...
      collected:
        totalCalculated > 0 ? Math.floor(totalCalculated * 0.7) : 1800000,
  ...
  const fetchStudentFinancials = async (studentId) => {
    ...
    try {
      ...
      const summary = {
        ...
        pendingFees: studentFees.reduce(
          (sum, fee) => sum + fee.pending_amount,
          0,
        ),
        overdueFees: studentFees.reduce(
          (sum, fee) => sum + fee.overdue_amount,
          0,
        ),
        lastPaymentDate:
          studentPaymentsData[0]?.payment_date?.split("T")[0] || null,
      };
      ...
      const overdueFees = studentFees
        .filter((fee) => fee.overdue_amount > 0)
        .map((fee) => ({
          id: `overdue_${fee.id}`,
          receiptNo: "OVERDUE",
          date: null,
  ...
  const getEligibleStudentsForBulkPayment = () => {
    return students.filter((student) => {
      // Filter by class
      if (
        bulkPaymentFilters.class !== "all" &&
        student.class_id !== bulkPaymentFilters.class
      ) {
        return false;
      }
  ...
  const calculateBulkPaymentSummary = () => {
    ...
    selectedStudents.forEach((student) => {
      ...
      const pendingAmount =
        studentFee?.pending_amount || studentFee?.total_due || 0;
  ...
  const submitBulkPayment = async (bulkPaymentData) => {
    ...
    try {
      ...
      const errorMessage =
        ...
        "Unable to process bulk payment. Please try again.";
  ...
  const handleExportReport = async (
    ...
    try {
      ...
      const response = await fetch(
        `${API}/reports/export?format=${format}&report_type=${reportType}`,
        {
          method: "GET",
          headers: {
  ...
  const submitPayment = async (paymentData) => {
    ...
    try {
      ...
      const studentAdmissionNo = student
        ? student.admission_no
        : paymentResult.admission_no;
      ...
      await Promise.all([
        ...
        fetchStudentFinancials(
          paymentResult?.student_id ?? paymentData.student_id,
        ),
      ]);
  ...
  const deleteFeeConfiguration = async (configId) => {
    try {
      ...
      toast.success("✅ Fee Configuration Deleted", {
        description:
          "Fee configuration has been deleted successfully. Dashboard updated.",
        duration: 5000,
      });
    } catch (error) {
      ...
      const errorMessage =
        error.response?.data?.detail ||
        "Unable to delete fee configuration. Please try again.";
  ...
  const handleStudentCollectPayment = (student) => {
    ...
    const pendingAmount =
      student.pending_amount || student.amount || student.total_due || 0; // Use actual amount or 0
  ...
  const handleContactGuardian = (student) => {
    ...
    navigator.clipboard
      .writeText(student.guardian_phone)
      .then(() => {
  ...
  const calculateCollectionStats = () => {
    ...
    const todaysPayments = recentPayments.filter(
      (p) => new Date(p.created_at || Date.now()).toDateString() === today,
    );
    ...
    const avgPayment =
      todaysPayments.length > 0 ? todaysCollection / todaysPayments.length : 0;
  ...
  const handleCollectionFormSubmit = async () => {
    if (
      !collectionForm.student_id ||
      !collectionForm.fee_type ||
      !collectionForm.amount ||
      !collectionForm.payment_mode
    ) {
    ...
    try {
      ...
      const newPayment = {
        ...
        receipt_no:
          ...
          `RCP2025-${String(recentPayments.length + 1).padStart(3, "0")}`,
        time: new Date().toLocaleTimeString(),
  ...
  const saveConfiguration = async () => {
    ...
    if (
      ...
      !configForm.applyToClasses
    ) {
      toast.error(`❌ Missing Required Fields`, {
        description:
          "Please fill in Amount, Frequency, and Apply to Classes fields.",
        duration: 4000,
      });
      return;
    }
    ...
    try {
      ...
      if (isEditing) {
        ...
        response = await axios.put(
          `${API}/fees/configurations/${editingConfig.id}`,
          configData,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
      ...
      toast.success(
        isEditing
          ? `✅ ${currentFeeType} Configuration Updated`
          : `✅ ${currentFeeType} Configuration Saved`,
        {
          description: isEditing
            ? `Fee structure updated successfully.`
            : `Fee structure saved successfully and applied to students.`,
          duration: 5000,
        },
      );
    } catch (error) {
      ...
      const errorMessage =
        error.response?.data?.detail ||
        "Unable to save fee configuration. Please try again.";
      toast.error(
        isEditingError
  ...
  return (
    <div className="p-3 sm:p-4 lg:p-6 space-y-3 sm:space-y-4 lg:space-y-6 fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        <div>
          <h1 className="text-sm sm:text-base md:text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white">
            {isMadrasahSimpleUI ? "বেতন ব্যবস্থাপনা" : "Fees Management"}
          </h1>
          <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mt-1">
            {isMadrasahSimpleUI
              ? "মাসিক বেতন আদায় ও হিসাব"
        ...
        <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-3">
          {!isMadrasahSimpleUI && (
            <Button
              variant="outline"
              size="sm"
              className="w-full sm:w-auto text-sm sm:text-base h-8 sm:h-9"
              ...
              <CardContent className="p-3 sm:p-4 md:p-6">
                <div className="flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <p className="text-lg sm:text-2xl md:text-3xl font-bold text-gray-900 truncate">
                      {getCurrencySymbol()}
              ...
              <CardContent className="p-3 sm:p-4 md:p-6">
                <div className="flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <p className="text-lg sm:text-2xl md:text-3xl font-bold text-emerald-600 truncate">
                      {getCurrencySymbol()}
                      {(collected / 100000).toFixed(1)}L
                    </p>
                    <p className="text-[10px] sm:text-xs text-emerald-500 hidden sm:block">
                      +15% this month
                    </p>
                  </div>
              ...
              <CardContent className="p-3 sm:p-4 md:p-6">
                <div className="flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <p className="text-lg sm:text-2xl md:text-3xl font-bold text-orange-600 truncate">
                      {getCurrencySymbol()}
                      {(pending / 100000).toFixed(1)}L
                    </p>
                    <p className="text-[10px] sm:text-xs text-orange-500 hidden sm:block">
                      Due this month
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <p className="text-lg sm:text-2xl md:text-3xl font-bold text-red-600 truncate">
                      {getCurrencySymbol()}
                      {(overdue / 100000).toFixed(1)}L
                    </p>
                    <p className="text-[10px] sm:text-xs text-red-500 hidden sm:block">
                      Needs attention
                    </p>
                  </div>
            ...
            <Card className="border-2 border-emerald-200">
              <CardHeader className="bg-emerald-50 p-4 sm:p-6">
                ...
                <p className="text-sm text-emerald-600 mt-1">
                  মারহালা ও শাখা বাছাই করে ছাত্র খুঁজুন
                </p>
              </CardHeader>
                  ...
                  <div>
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        {classes.map((cls) => (
                          <SelectItem
                            key={cls.id || cls._id || cls.name}
                            value={cls.name || cls.class_name}
                          >
                      ...
                      <SelectContent>
                        ...
                        {sections.map((section) => (
                          <SelectItem
                            ...
                            value={section.name || section.section_name}
                          >
                        ...
                        students.filter(
                          (s) =>
                            (selectedClass === "all" ||
                              ...
                              getClassName(s.class_id) === selectedClass) &&
                            (selectedSection === "all" ||
                    ...
                    {students.filter(
                      ...
                      students
                        ...
                        .map((student) => {
                          ...
                          const studentFeeRecords = dueFees.filter(
                            (f) => f.student_id === student.id,
                          );
                          const studentDue =
                            studentFeeRecords.length > 0
                              ? {
                                  paid_amount: studentFeeRecords.reduce(
                                    (sum, f) => sum + (f.paid_amount || 0),
                                    0,
                                  ),
                                  pending_amount: studentFeeRecords.reduce(
                                    (sum, f) => sum + (f.pending_amount || 0),
                                    0,
                                  ),
                                  overdue_amount: studentFeeRecords.reduce(
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="flex items-center gap-3">
                                  ...
                                  <div className="text-right text-sm">
                                    {studentDue.paid_amount > 0 && (
                                      <p className="text-green-600">
                                        পেইড: ৳
                                        {studentDue.paid_amount?.toLocaleString() ||
                                          0}
                                      </p>
                                    )}
                                    {hasDue && (
                                ...
                                <Badge
                                  className={`text-sm px-3 py-1 ${hasDue ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}
                                >
              ...
              <CardHeader className="bg-emerald-50 p-4 sm:p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-emerald-600 mt-1">
                      {selectedStudent.name || selectedStudent.student_name} এর
                      বেতন
                    </p>
                  </div>
                  <Button
                ...
                <div className="bg-gray-50 rounded-lg p-4 mb-6 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600">
                      {getClassName(selectedStudent.class_id)} | রোল:{" "}
                      {selectedStudent.roll_no || selectedStudent.roll || "-"}
                    </p>
                    <p className="text-gray-500 text-sm">
                      ভর্তি নং: {selectedStudent.admission_no || "-"}
                    </p>
                  </div>
                </div>
              ...
              <CardContent className="p-4 sm:p-6">
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      backgroundColor:
                        schoolBranding.primary_color || "#059669",
                    }}
                  >
                  ...
                  <div className="p-4 space-y-3 text-sm">
                    ...
                    <div className="flex justify-between items-center pb-2 border-b border-dashed border-gray-300">
                      ...
                      <div className="text-right">
                        ...
                        <p className="font-medium text-gray-800">
                          {lastReceipt.academic_year ||
                            `${new Date().getFullYear()} খ্রি.`}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-50 rounded-lg p-3 space-y-2">
                      <h4 className="font-bold text-gray-700 text-xs uppercase border-b pb-1">
                        ছাত্র তথ্য
                      </h4>
                        ...
                        <div>
                          ...
                          <p className="font-bold text-gray-900">
                            {lastReceipt.student?.name ||
                              lastReceipt.student?.student_name ||
                              "অজ্ঞাত"}
                          </p>
                        </div>
                        <div>
                          ...
                          <p className="font-bold text-gray-900">
                            {lastReceipt.student?.roll_no ||
                              lastReceipt.student?.roll ||
                              "-"}
                          </p>
                        </div>
                        <div>
                          ...
                          <p className="font-bold text-gray-900">
                            {getClassName(lastReceipt.student?.class_id)}
                          </p>
                        </div>
                        <div>
                          <span className="text-gray-500 text-xs">
                    ...
                    <div className="bg-emerald-50 rounded-lg p-3 space-y-2">
                      <h4 className="font-bold text-emerald-700 text-xs uppercase border-b border-emerald-200 pb-1">
                        বেতন তথ্য
                      </h4>
                      <div className="space-y-2">
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium text-gray-800">
                            {lastReceipt.fee_type === "Tuition Fees"
                              ? "মাসিক বেতন"
                              : lastReceipt.fee_type === "Transport Fees"
                                ? "পরিবহন ফি"
                                : lastReceipt.fee_type === "Admission Fees"
                                  ? "ভর্তি ফি"
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium text-gray-800">
                            {lastReceipt.payment_mode === "Cash"
                              ? "নগদ"
                              : lastReceipt.payment_mode === "bKash"
                                ? "বিকাশ"
                                : lastReceipt.payment_mode === "Nagad"
                                  ? "নগদ"
                                  : lastReceipt.payment_mode === "Rocket"
                                    ? "রকেট"
                                    : lastReceipt.payment_mode === "Bank"
                                      ? "ব্যাংক"
                  ...
                  <div className="text-center py-3 px-4 border-t-2 border-gray-300 bg-gray-100">
                    <p className="text-xs text-gray-500 mb-1">
                      এই রসিদটি কম্পিউটার জেনারেটেড এবং অফিস কপি হিসেবে বৈধ।
                    </p>
                    <p className="text-[10px] text-gray-400">
                      Office Copy | Computer Generated Receipt
                    </p>
        ...
        >
          <TabsList className="grid w-full grid-cols-3 sm:grid-cols-5 h-auto">
            ...
            <TabsTrigger
              ...
              className="text-xs sm:text-sm py-2 px-1 sm:px-3 hidden sm:flex"
            >
              Select
            </TabsTrigger>
            <TabsTrigger
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                  ...
                  <Card className="border-2 border-dashed border-gray-300 hover:border-emerald-500 transition-colors min-w-0">
                    <CardContent className="p-4 sm:p-6 text-center">
                      ...
                      <p className="text-xs sm:text-sm text-gray-600 mb-2 sm:mb-3">
                        Bus and transport charges
                      </p>
                      <Button
                        variant="outline"
                        size="sm"
                        className="w-full sm:w-auto text-xs sm:text-sm"
                        onClick={() => handleFeeConfiguration("Transport Fees")}
                      >
                        Configure
                      </Button>
                    </CardContent>
                  ...
                  <Card className="border-2 border-dashed border-gray-300 hover:border-emerald-500 transition-colors min-w-0 sm:col-span-2 lg:col-span-1">
                    <CardContent className="p-4 sm:p-6 text-center">
                      ...
                      <Button
                        ...
                        size="sm"
                        className="w-full sm:w-auto text-xs sm:text-sm"
                        onClick={() => handleFeeConfiguration("Admission Fees")}
                      >
                        Configure
                      </Button>
                    </CardContent>
                ...
                <CardContent className="p-3 sm:p-6">
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4">
                    ...
                    <div className="text-center p-2 sm:p-4 bg-blue-50 rounded-lg">
                      ...
                      <p className="text-[10px] sm:text-xs md:text-sm text-blue-600">
                        Today's Collection
                      </p>
                    </div>
                    <div className="text-center p-2 sm:p-4 bg-orange-50 rounded-lg">
                      ...
                      <p className="text-[10px] sm:text-xs md:text-sm text-orange-600">
                        Pending
                      </p>
                    </div>
                    <div className="text-center p-2 sm:p-4 bg-purple-50 rounded-lg">
                      <p className="text-lg sm:text-xl md:text-2xl font-bold text-purple-600">
                        {getCurrencySymbol()}
                        {(monthlyTarget / 100000).toFixed(1)}L
                      </p>
                      <p className="text-[10px] sm:text-xs md:text-sm text-purple-600">
                        Monthly Target
                      </p>
                    </div>
                  </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <Button
                      className="h-20 flex flex-col gap-2 bg-emerald-500 hover:bg-emerald-600"
          ...
          <TabsContent value="student-specific" className="space-y-6">
            ...
            <div className="bg-gradient-to-r from-emerald-50 to-green-50 border-l-4 sm:border-l-8 border-emerald-500 rounded-lg p-4 sm:p-6 md:p-8 mb-4 sm:mb-6">
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <div>
                  <h1 className="text-base sm:text-2xl md:text-4xl font-bold text-emerald-700">
                    👨‍🎓 STUDENT SPECIFIC TAB
                  </h1>
                  <h2 className="text-xs sm:text-lg md:text-xl font-bold text-emerald-700">
                    Individual Student Financial Reports
                  </h2>
                  <p className="text-[10px] sm:text-sm md:text-base text-emerald-600">
                    Individual student fee management, payment history, and
                    reports
                  </p>
                </div>
              </div>
            ...
            <div
              ...
              data-section="student-list-details"
            >
              {/* Student List */}
                ...
                {!selectedStudent ? (
                  <Card>
                    <CardContent className="p-12 text-center">
                      ...
                      <p className="text-gray-600">
                        Choose a student from the list to view their financial
                        details
                      </p>
                    </CardContent>
                  </Card>
                        ...
                        <CardContent>
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div className="text-center p-4 bg-blue-50 rounded-lg">
                              <p className="text-2xl font-bold text-blue-600">
                                {formatCurrency(studentFeesSummary.totalFees)}
                              </p>
                            ...
                            <div className="text-center p-4 bg-green-50 rounded-lg">
                              <p className="text-2xl font-bold text-green-600">
                                {formatCurrency(studentFeesSummary.paidAmount)}
                              </p>
                            ...
                            <div className="text-center p-4 bg-yellow-50 rounded-lg">
                              <p className="text-2xl font-bold text-yellow-600">
                                {formatCurrency(studentFeesSummary.pendingFees)}
                              </p>
                            ...
                            <div className="text-center p-4 bg-red-50 rounded-lg">
                              <p className="text-2xl font-bold text-red-600">
                                {formatCurrency(studentFeesSummary.overdueFees)}
                              </p>
                          ...
                          <div className="mt-6">
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <span className="text-sm text-gray-500">
                                {(
                                  (studentFeesSummary.paidAmount /
                                    (studentFeesSummary.totalFees || 1)) *
                                  100
                                ).toFixed(1)}
                                % Complete
                              </span>
                      ...
                      <CardContent>
                        <div className="flex flex-wrap gap-3">
                          ...
                          <Button
                            variant="outline"
                            onClick={() =>
                              handleViewAllReceipts(selectedStudent)
                            }
                          >
                          ...
                          <Button
                            variant="outline"
                            onClick={() =>
                              handleContactGuardian(selectedStudent)
                            }
                          >
                            <Phone className="h-4 w-4 mr-2" />
                            Contact Guardian
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-red-600">
                    Track and collect pending fees, send reminders, manage
                    overdue accounts
                  </p>
                </div>
              </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-orange-500">
                        {dueFees.filter((f) => f.status === "pending").length}{" "}
                        students
                      </p>
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-red-500">
                        {dueFees.filter((f) => f.status === "overdue").length}{" "}
                        students
                      </p>
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-yellow-500">
                        Total: {formatCurrency(pending + overdue)}
                      </p>
                    </div>
                ...
                <CardTitle className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        handleExportReport("excel", "overdue_students")
                      }
                    >
                      <Download className="h-4 w-4 mr-2" />
                      Export List
                        ...
                        dueFees
                          ...
                          .map((fee, index) => (
                            ...
                            >
                              ...
                              <TableCell>
                                {getClassName(fee.class_id) || "Unknown"}
                              </TableCell>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-purple-600">
                    Quick student lookup with fee collection and profile access
                  </p>
                </div>
              </div>
            </div>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      toast.success("🔍 Advanced Search", {
                        description:
                          "Use the Class and Section filters above to refine your search results.",
                        duration: 4000,
                      });
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {filteredStudents.slice(0, 9).map((student) => (
                    <Card
                      key={student.id}
                      className="cursor-pointer hover:shadow-lg transition-all border-2 hover:border-purple-200 group"
                    >
                        ...
                        <div className="space-y-2 mb-4">
                          <div className="flex justify-between items-center text-sm">
                            ...
                            <span className="text-orange-600 font-medium">
                              {getCurrencySymbol()}12,500
                            </span>
                          </div>
                          <div className="flex justify-between items-center text-sm">
                            <span className="text-gray-600">Overdue:</span>
                            <span className="text-red-600 font-medium">
                              {getCurrencySymbol()}3,200
                            </span>
                          </div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <Button
                            ...
                            onClick={() => {
                              ...
                              const studentListSection = document.querySelector(
                                '[data-section="student-list-details"]',
                              );
          ...
          <TabsContent value="collection" className="space-y-6">
            ...
            <div
              className={`bg-gradient-to-r ${isMadrasahSimpleUI ? "from-emerald-50 to-green-50 border-emerald-500" : "from-green-50 to-emerald-50 border-green-500"} border-l-8 rounded-lg p-6 sm:p-8 mb-6`}
            >
              <div className="flex items-center gap-3">
                <div
                  className={`${isMadrasahSimpleUI ? "bg-emerald-500" : "bg-green-500"} text-white rounded-full p-3 sm:p-4`}
                >
                  <CreditCard className="h-6 w-6 sm:h-8 sm:w-8" />
                </div>
                <div>
                  <h1
                    className={`text-2xl sm:text-3xl md:text-4xl font-bold ${isMadrasahSimpleUI ? "text-emerald-700" : "text-green-700"}`}
                  >
                    {isMadrasahSimpleUI
                      ? "💰 বেতন আদায়"
                      : "💳 FEE COLLECTION TAB"}
                  ...
                  <p
                    className={`text-sm sm:text-base ${isMadrasahSimpleUI ? "text-emerald-600" : "text-green-600"}`}
                  >
                    {isMadrasahSimpleUI
                      ? "মারহালা → শাখা → ছাত্র নির্বাচন → বেতন আদায়"
                      : "Process payments, generate receipts, and track daily collections"}
                  </p>
                </div>
              </div>
            </div>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-green-600">
                          {getCurrencySymbol()}
                          {collectionStats.todaysCollection
                            ? (collectionStats.todaysCollection / 1000).toFixed(
                                0,
                              ) + "K"
                            : "0"}
                        </p>
                        <p className="text-xs text-green-500">
                          {collectionStats.transactions || 0} payments today
                        </p>
                      </div>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-blue-600">
                          {collectionStats.transactions || 0}
                        </p>
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-gray-500">
                                  ...
                                  রোল: {student.roll_no || student.roll || "-"}
                                </p>
                              </div>
                            </div>
                            <div className="text-right">
                          ...
                          <div className="border border-green-200 bg-green-50 rounded-lg p-4">
                            <div className="text-center mb-4">
                              ...
                              <p className="font-bold text-green-800">
                                Receipt #{receiptPreview.receiptNo}
                              </p>
                              <p className="text-xs text-green-600">
                                {receiptPreview.date} • {receiptPreview.time}
                              </p>
                            </div>
                            {receiptPreview.student && (
                              <div className="space-y-1 text-sm">
                                <p>
                                  ...
                                  {receiptPreview.student.name}
                                </p>
                                <p>
                                  <strong>Admission:</strong>{" "}
                                  {receiptPreview.student.admission_no}
                                </p>
                                <p>
                                  <strong>Class:</strong>{" "}
                                  {receiptPreview.student.class}
                                </p>
                                <p>
                                  <strong>Fee Type:</strong>{" "}
                                  {receiptPreview.fee_type}
                                </p>
                                <p>
                          ...
                          <Button
                            ...
                            onClick={() => {
                              ...
                              const student = students.find(
                                (s) => s.id === collectionForm.student_id,
                              );
                              setSelectedStudent(student);
                              fetchStudentFinancials(student.id);
                              handleTabChange("student-specific");
                          ...
                          <Button
                            ...
                            onClick={() => {
                              if (!collectionForm.student_id) {
                                toast.error("⚠️ Select Student First", {
                                  description:
                                    "Please select a student to contact their parent.",
                                  duration: 3000,
                                });
                                return;
                              }
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-500">
                                {payment.time} • {payment.payment_mode}
                              </p>
                            </div>
                          </div>
                          <div className="text-right">
      ...
      <Dialog open={showFeeConfigModal} onOpenChange={setShowFeeConfigModal}>
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Manage fee structures, amounts, and payment schedules for{" "}
              {currentFeeType}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-6">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setConfigForm((prev) => ({ ...prev, frequency: value }))
                      }
                    >
                      <SelectTrigger>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Select
                      value={configForm.applyToClasses}
                      onValueChange={(value) =>
                        setConfigForm((prev) => ({
                          ...prev,
                          applyToClasses: value,
                        }))
                      }
                    >
                      <SelectTrigger>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">
                  Current {currentFeeType} Structure
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="rounded-md border">
                  <table className="w-full">
                    ...
                    <tbody>
                      {feeConfigurations[currentFeeType] &&
                      feeConfigurations[currentFeeType].length > 0 ? (
                        feeConfigurations[currentFeeType].map(
                          (config, index) => {
                            // Get class name from apply_to_classes field
                            let className = "All Classes";
                            if (
                              config.apply_to_classes &&
                              config.apply_to_classes !== "all"
                            ) {
                              const classObj = classes.find(
                                (c) => c.id === config.apply_to_classes,
                              );
                              className = classObj
                                ? `${classObj.name} (${classObj.standard})`
                                : config.apply_to_classes;
                            }
                        ...
                        <tr>
                          ...
                          >
                            No fee configurations found. Click "Save
                            Configuration" to add new fees.
                          </td>
                        </tr>
                      )}
                    </tbody>
          ...
          {configToDelete && (
            <div className="bg-gray-50 p-4 rounded-lg">
              <div className="text-sm space-y-1">
                <p>
                  <strong>Fee Type:</strong> {currentFeeType}
                </p>
                <p>
                  <strong>Amount:</strong>{" "}
                  {formatCurrency(configToDelete.amount)}
                </p>
                <p>
                  <strong>Frequency:</strong> {configToDelete.frequency}
                </p>
                <p>
                  <strong>Classes:</strong>{" "}
                  {(() => {
                    if (
                      configToDelete.apply_to_classes === "all" ||
                      !configToDelete.apply_to_classes
                    )
                      return "All Classes";
                    const classObj = classes.find(
                      (c) => c.id === configToDelete.apply_to_classes,
                    );
                    return classObj
                      ? `${classObj.name} (${classObj.standard})`
                      : configToDelete.apply_to_classes;
      ...
      <Dialog
        open={showGenerateReportsModal}
        onOpenChange={setShowGenerateReportsModal}
      >
        <DialogContent className="max-w-3xl">
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <Button
                    className="h-16 flex flex-col gap-2 bg-red-500 hover:bg-red-600"
                    onClick={() => {
                      setShowGenerateReportsModal(false);
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setShowGenerateReportsModal(false)}
            >
              Cancel
            </Button>
            <Button
        ...
        <DialogContent className="max-w-5xl">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="space-y-2">
                      ...
                      <Select
                        value={bulkPaymentFilters.class}
                        onValueChange={(value) =>
                          setBulkPaymentFilters((prev) => ({
                            ...prev,
                            class: value,
                          }))
                        }
                    ...
                    <div className="space-y-2">
                      <label className="text-sm font-medium">Fee Type</label>
                      <Select
                        value={bulkPaymentFilters.feeType}
                        onValueChange={(value) =>
                          setBulkPaymentFilters((prev) => ({
                            ...prev,
                            feeType: value,
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="Tuition Fees">
                            Tuition Fees
                          </SelectItem>
                          <SelectItem value="Transport Fees">
                            Transport Fees
                          </SelectItem>
                          <SelectItem value="Admission Fees">
                            Admission Fees
                          </SelectItem>
                        </SelectContent>
                      </Select>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      >
                        ...
                        getEligibleStudentsForBulkPayment().length
                          ? "Deselect All"
                          : "Select All"}
                      </Button>
                    </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                          <div>
                            ...
                            <p className="font-bold text-emerald-800">
                              {calculateBulkPaymentSummary().studentsCount}
                            </p>
                          </div>
                          <div>
                            ...
                            <p className="font-bold text-emerald-800">
                              {formatCurrency(
                                calculateBulkPaymentSummary().totalAmount,
                              )}
                            </p>
                          </div>
                          <div>
                            ...
                            <p className="font-bold text-emerald-800">
                              {formatCurrency(
                                calculateBulkPaymentSummary().lateFees,
                              )}
                            </p>
                          </div>
                          <div>
                            ...
                            <p className="font-bold text-emerald-800">
                              {formatCurrency(
                                calculateBulkPaymentSummary().grandTotal,
                              )}
                            </p>
                          </div>
                        </div>
          ...
          <DialogFooter className="gap-2">
            ...
            >
              {loading
                ? "Processing..."
                : `Collect Payment (${formatCurrency(calculateBulkPaymentSummary().grandTotal)})`}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      >
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Send automated reminders to students with pending or overdue fee
              payments
            </DialogDescription>
          </DialogHeader>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                  ...
                  <div className="text-sm text-orange-700">
                    <p>
                      <strong>Subject:</strong> Fee Payment Reminder - School
                      ERP
                    </p>
                    <p>
                      <strong>Message:</strong> Dear Student/Parent, this is a
                      reminder about pending fee payments. Please make the
      ...
      if (student && student.class_id) {
        ...
        const matchingConfig = configs.find((config) => {
          console.log("🔎 Checking config:", {
            ...
            match:
              config.apply_to_classes === student.class_id ||
              config.apply_to_classes === "all",
          });
          return (
            config.apply_to_classes === student.class_id ||
        ...
        if (matchingConfig && matchingConfig.amount) {
          ...
          console.log(
            `✅ Auto-filled amount: ${matchingConfig.amount} for ${feeType} (${student.name})`,
          );
        } else {
          console.warn(
            "❌ No matching fee configuration found for this student/class",
          );
        }
      } else {
      ...
      <div className="space-y-2">
        ...
        <Select value={feeType} onValueChange={setFeeType}>
          ...
          <SelectContent>
            ...
            {Object.keys(feeConfigurations || {}).map(
              (feeType) =>

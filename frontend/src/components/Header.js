  ...
  const fetchHeaderData = useCallback(async () => {
    ...
    try {
      ...
      const fetchPromise = Promise.all([
        axios
          .get(`${API_BASE_URL}/notifications?limit=5`, {
            headers: { Authorization: `Bearer ${token}` },
            signal,
          })
          .catch(() => ({ data: [] })),
        axios
          .get(`${API_BASE_URL}/notifications/unread-count`, {
            headers: { Authorization: `Bearer ${token}` },
            signal,
          })
          .catch(() => ({ data: { unread_count: 0 } })),
        axios
          .get(`${API_BASE_URL}/attendance/summary?type=student`, {
            headers: { Authorization: `Bearer ${token}` },
            signal,
          })
      ...
      const [notifRes, countRes, attendRes] = await Promise.race([
        fetchPromise,
        timeoutPromise,
      ]);

      const notifData = notifRes.data;
      const notificationsArray = Array.isArray(notifData)
        ? notifData
      ...
      if (
        ...
        error.message !== "Header fetch timeout"
      ) {
    ...
    <header className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-2 sm:px-4 md:px-6 py-2 sm:py-3 md:py-4">
      ...
      <div className="hidden sm:flex items-center space-x-4 sm:space-x-6 mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-gray-100 dark:border-gray-800">
        <div className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 flex flex-wrap items-center gap-1 sm:gap-0">
          <span className="font-medium">আজকে:</span>
          <span className="ml-1 sm:ml-2 text-emerald-600 dark:text-emerald-400">
            {attendanceSummary.present} উপস্থিত
          </span>
          <span className="mx-1 sm:mx-2">•</span>
          <span className="text-red-600 dark:text-red-400">
            {attendanceSummary.absent} অনুপস্থিত
          </span>
          <span className="mx-1 sm:mx-2">•</span>
          <span className="text-orange-600 dark:text-orange-400">
            {attendanceSummary.late} দেরি
          </span>
        </div>

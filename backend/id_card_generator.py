"""
Professional Madrasah ID Card Generator
Supports both Bengali and English text rendering using PIL.
Compact layout with no blank space.
"""
...
def draw_text(c, x, y, text, font_size, color=(0, 0, 0), bold=False, centered=False, width=None):
    """Render text using PIL - supports both Bengali and English"""
    ...
    try:
        ...
        logging.error(f"Text render failed: {e}")
        return 0
...
def draw_circuit_pattern(c, width, height):
    ...
    for i in range(0, int(height), int(12*mm)):
        c.circle(2*mm, i + 6*mm, 1.5*mm, fill=True, stroke=False)
        c.circle(width - 2*mm, i + 6*mm, 1.5*mm, fill=True, stroke=False)
...
def generate_front_side(c, person, institution, class_name=""):
    """Generate front side - compact layout, no blank space"""
    margin = 2.5 * mm
    ...
    y_pos = CARD_HEIGHT - margin - logo_size - 2*mm
    
    if name_bn:
        draw_text(c, 0, y_pos, name_bn, 5, color=(0, 100, 0), bold=True, centered=True, width=CARD_WIDTH)
        y_pos -= 4*mm
    
    if name_en:
        draw_text(c, 0, y_pos, name_en.upper(), 3.5, color=(0, 100, 0), bold=True, centered=True, width=CARD_WIDTH)
        y_pos -= 3*mm
    
    if established:
        draw_text(c, 0, y_pos, f"স্থাপিত: {established} ইং", 3, color=(0, 0, 0), centered=True, width=CARD_WIDTH)
        y_pos -= 3*mm
    
    if address:
        addr_short = address[:35] if len(address) > 35 else address
        draw_text(c, 0, y_pos, addr_short, 3, color=(51, 51, 51), centered=True, width=CARD_WIDTH)
        y_pos -= 4*mm
    
    photo_width = 16 * mm
    photo_height = 20 * mm
    photo_x = (CARD_WIDTH - photo_width) / 2
    photo_y = y_pos - photo_height
    photo_url = person.get("photo") or person.get("photo_url") or person.get("profile_image") or None
    ...
    position = person.get("designation") or person.get("position") or class_name or ""
    student_address = person.get("address") or person.get("permanent_address") or ""
    joining_date = person.get("joining_date") or person.get("admission_date") or ""
    dob = person.get("date_of_birth") or person.get("dob") or ""
    mobile = person.get("phone") or person.get("mobile") or person.get("contact") or ""
    guardian_mobile = person.get("guardian_phone") or person.get("guardian_mobile") or person.get("parent_phone") or ""
    
    is_student = person.get("roll_no") or person.get("admission_no") or person.get("class_id")
    
    y_pos = photo_y - 3*mm
    draw_text(c, 0, y_pos, f"নাম: {name}", 6, color=(0, 0, 0), bold=True, centered=True, width=CARD_WIDTH)
    y_pos -= 4*mm
    
    if position:
        label = "শ্রেণী" if is_student else "পদের নাম"
        draw_text(c, 0, y_pos, f"{label}: {position}", 4, color=(139, 0, 0), bold=True, centered=True, width=CARD_WIDTH)
        y_pos -= 3.5*mm
    ...
    for label, value in details:
        if value:
            draw_text(c, 0, y_pos, f"{label}: {value}", 3.5, color=(51, 51, 51), centered=True, width=CARD_WIDTH)
            y_pos -= line_height
    
    sig_y = 8*mm
    c.setStrokeColor(GRAY_LINE)
    c.setDash([1, 1])
    c.line(margin + 10*mm, sig_y, CARD_WIDTH - margin - 10*mm, sig_y)
    c.setDash([])
    
    draw_text(c, 0, sig_y - 3.5*mm, "প্রধান শিক্ষক", 4, color=(0, 0, 0), bold=True, centered=True, width=CARD_WIDTH)
    
    draw_red_curved_footer(c, CARD_WIDTH, 3.5*mm)


def generate_back_side(c, person, institution):
    """Generate back side - compact layout"""
    margin = 2.5 * mm
    ...
    for line in message_lines:
        draw_text(c, 0, y_pos, line, 3.5, color=(51, 51, 51), centered=True, width=CARD_WIDTH)
        y_pos -= 3*mm
    ...
    person_id = person.get("id") or person.get("student_id") or person.get("admission_no") or ""
    name = person.get("name") or person.get("full_name") or ""
    qr_data = f"ID:{person_id}|Name:{name}"
    ...
    if phone:
        draw_text(c, 0, y_pos, f"মোবাইল: {phone}", 3.5, color=(0, 0, 0), bold=True, centered=True, width=CARD_WIDTH)
